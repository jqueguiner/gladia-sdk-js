...
function getEndpoints() {
    return Object.entries(openapidef_1.openApiJson.paths).map(function (_a) {
        ...
        return __assign(__assign(__assign({ url: path, inputType: inputType, outputType: outputType, taskName: taskName }, getPostModels(def)), getInputBodyContentType(inputType)), { params: getPostParams(def, openapidef_1.openApiJson) });
    });
}
...
function getInputBodyContentType(inputType) {
    switch (inputType) {
        ...
        default:
            throw { kind: 'InvalidInputType', inputType: inputType };
    }
}
